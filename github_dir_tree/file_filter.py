...
class FileFilter:
    ...
    def get_filter_files(self) -> list:
        ...
        if self._filter_files is None:
            ...
            spec = pathspec.PathSpec.from_lines('gitwildmatch', self.git_wild_match)
            matches = spec.match_tree(self._project_root)
            self._filter_files = []
            for file in matches:
                self._filter_files.append(os.path.join(self._project_root, file))
        return self._filter_files

    def is_match(self, abs_path):
        if not self.is_in_project(abs_path):
            return False
        spec = pathspec.PathSpec.from_lines(pathspec.patterns.GitWildMatchPattern, self.git_wild_match)
